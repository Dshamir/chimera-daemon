...
class TelemetryDashboard:
    ...
    def __init__(self, stats: dict, entity_counts: Optional[Dict] = None):
        self.stats = stats
        self.entity_counts = entity_counts or {
            "EMAIL": 0, "URL": 0, "PATH": 0, 
            "DATE": 0, "PROPER_NOUN": 0, "GPS": 0, "CAMERA": 0,
        }
        self.start_time = datetime.now()
        self.current_file = ""
        self.feed = deque(maxlen=10)
        self.velocity_samples = deque(maxlen=60)
        self.last_count = 0
    ...
    def get_velocity(self) -> float:
        ...
        if elapsed >= 1.0:
            # Total processed = text + images + audio
            current_count = (
                self.stats.get("files_processed", 0) +
                self.stats.get("images_processed", 0) +
                self.stats.get("audio_registered", 0)
            )
    ...
    def _header_panel(self) -> Panel:
        ...
        return Panel(
            f"{status}  │  Uptime: {self.get_uptime()}  │  Velocity: {velocity:.0f}/min  │  Total: {total:,}",
    ...
    def _velocity_panel(self) -> Panel:
        ...
        elapsed = max(1, (datetime.now() - self.start_time).total_seconds())
        
        content = f"""
[cyan]Files/min:[/cyan]  {velocity:>6.0f}  {spark}
[cyan]Chunks/s:[/cyan]   {self.stats.get('chunks_created', 0) / elapsed:>6.1f}
[cyan]Entities/s:[/cyan] {self.stats.get('entities_extracted', 0) / elapsed:>6.1f}
"""
    ...
    def _stats_panel(self) -> Panel:
        """Main statistics with media breakdown."""
        files_found = self.stats.get("files_found", 0)
        text_files = self.stats.get("files_processed", 0)
        images = self.stats.get("images_processed", 0)
        audio = self.stats.get("audio_registered", 0)
        total_processed = text_files + images + audio
        chunks = self.stats.get("chunks_created", 0)
        entities = self.stats.get("entities_extracted", 0)
        errors = self.stats.get("errors", 0)
        
        if files_found > 0:
            progress = total_processed / files_found
            bar = self.make_bar(total_processed, files_found, 15)
        else:
            progress = 0
...
[cyan]Processed:[/cyan]       {total_processed:>10,}  {bar} {progress*100:>5.1f}%
──────────────────────────────────
[cyan]📄 Documents:[/cyan]    {text_files:>10,}
[yellow]📷 Images:[/yellow]       {images:>10,}
[magenta]🎵 Audio:[/magenta]        {audio:>10,}
──────────────────────────────────
[green]🧩 Chunks:[/green]       {chunks:>10,}
[yellow]🏷️  Entities:[/yellow]     {entities:>10,}
...
"""
    ...
    def _entities_panel(self) -> Panel:
        ...
        active = {k: v for k, v in self.entity_counts.items() if v > 0}
        
        if not active:
            return Panel("[dim]No entities yet...[/dim]", title="Entity Types", border_style="yellow")
        
        max_count = max(active.values()) if active else 1
        
        lines = []
        for etype, count in sorted(active.items(), key=lambda x: -x[1]):
...
class SimpleTelemetry:
    ...
    def update(self, current_file: str):
        ...
        velocity = total / elapsed * 60 if elapsed > 0 else 0
        
        short = current_file[-50:] if len(current_file) > 50 else current_file
        print(f"\r[{total:>6}] {velocity:>5.0f}/min │ {short:<50}", end="", flush=True)
